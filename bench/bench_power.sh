...
# Sample CPU + GPU power and ANE residency at 500ms cadence. powermetrics
# emits one sample group per --sample-interval; we'll integrate after the run.
...
# Small grace period so powermetrics has at least one sample before the workload.
sleep 1
...
sudo -u "$RUN_USER" "$PY" -m bench.bench_throughput \
    --backend "$BACKEND" --prompt-id "$PROMPT_ID" \
    >"$RUN_LOG" 2>&1
...
# powermetrics emits "CPU Power: NNN mW", "GPU Power: NNN mW", "ANE Power: NNN mW"
# at the configured sample interval (500ms => 0.5s integration step).
SAMPLE_S = 0.5
total_j = {"cpu": 0.0, "gpu": 0.0, "ane": 0.0}
samples = {"cpu": 0, "gpu": 0, "ane": 0}

text = Path(pm_path).read_text(errors="replace")
for line in text.splitlines():
    m = re.match(r"\s*(CPU|GPU|ANE) Power:\s*([0-9]+)\s*mW", line)
    if not m:
        continue
    key = m.group(1).lower()
    mw = int(m.group(2))
...
print(f"    cpu: {total_j['cpu']:.1f} J   gpu: {total_j['gpu']:.1f} J   "
      f"ane: {total_j['ane']:.1f} J   total: {total_pkg_j:.1f} J ({wh*1000:.1f} mWh)")
